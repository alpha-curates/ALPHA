...
function ConfirmDialog({ open, title, message, onConfirm, onCancel }: {
  ...
  if (!open) return null
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 10000, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={onCancel}>
...
export default function PhotosPage() {
  ...
  const deletePhoto = async () => {
    ...
    try {
      await api.delete(`/photos/delete/${deleteTarget.id}`)
  ...
  const deleteAlbum = async () => {
    ...
    try {
      await api.delete(`/photos/albums/delete/${deleteAlbumTarget.id}`)
      setAlbums(prev => prev.filter(a => a.id !== deleteAlbumTarget.id))
      addToast('Album deleted', 'success')
    } catch { addToast('Failed to delete album', 'error') }
    finally { setDeleteAlbumTarget(null) }
  }

  const createAlbum = async () => {
    if (!newAlbumName.trim()) return
    try {
      const r = await api.post('/photos/albums/create', { name: newAlbumName.trim() })
      setAlbums(prev => [...prev, r.data])
      setNewAlbumName('')
      setShowCreateAlbum(false)
      addToast('Album created', 'success')
    } catch { addToast('Failed to create album', 'error') }
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <ConfirmDialog
        ...
        title="Delete Album"
        message={`Are you sure you want to delete the album "${deleteAlbumTarget?.name}"?`}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <h3 style={{ fontSize: 16, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 400 }}>({totalPhotos})</span>
        </h3>
        <div style={{ flex: 1 }} />
      ...
      {mode === 'albums' && (
        <>
          <div style={{ display: 'flex', gap: 8 }}>
            {showCreateAlbum ? (
              <div className="glass-card" style={{ padding: 12, display: 'flex', gap: 8, flex: 1 }}>
                ...
                <button className="btn btn-primary btn-sm" onClick={createAlbum}>Create</button>
                <button className="btn btn-ghost btn-sm" onClick={() => setShowCreateAlbum(false)}>Cancel</button>
              </div>
            ) : (
              <button className="btn btn-primary btn-sm" onClick={() => setShowCreateAlbum(true)}><Plus size={14} /> New Album</button>
            )}
          </div>
          ...
          {albums.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 8 }}>
              {albums.map(a => (
                <div key={a.id} className="card-liquid" style={{ padding: 16, cursor: 'pointer', textAlign: 'center', position: 'relative' }}>
                  <div onClick={() => { setMode('photos'); setSelectedAlbumId(a.id) }}>
                  ...
                  <button className="btn btn-ghost btn-icon btn-sm" onClick={e => { e.stopPropagation(); setDeleteAlbumTarget(a) }}
                    title="Delete Album" style={{ position: 'absolute', top: 8, right: 8, color: 'var(--danger)' }}>
                    <Trash2 size={12} />
                  </button>
                </div>
              ))}
      ...
      {mode === 'photos' && (
        filtered.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 6 }}>
            {filtered.map(p => (
              <div key={p.id} className="card-liquid" style={{ padding: 0, overflow: 'hidden', cursor: 'pointer', aspectRatio: '1', position: 'relative' }}>
                ...
                <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setDeleteTarget(p)}
                  title="Delete" style={{ position: 'absolute', top: 6, right: 6, color: 'var(--danger)', background: 'rgba(0,0,0,0.5)' }}>
                  <Trash2 size={12} />
                </button>
              </div>
            ))}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {filtered.map(p => (
              <div key={p.id} className="glass-card" style={{ padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 40, height: 40, borderRadius: 6, overflow: 'hidden', flexShrink: 0, background: 'var(--glass-bg)', cursor: 'pointer' }} onClick={() => setPreview(p)}>
                  <img src={`/api/photos/thumbnail/${p.id}`} alt={p.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </div>
                <div style={{ flex: 1, minWidth: 0, cursor: 'pointer' }} onClick={() => setPreview(p)}>
                  <div style={{ fontSize: 13, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}</div>
                </div>
                <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setDeleteTarget(p)} title="Delete" style={{ color: 'var(--danger)' }}><Trash2 size={12} /></button>
              </div>
            ))}
          </div>
        )
