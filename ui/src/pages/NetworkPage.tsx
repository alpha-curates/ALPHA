...
import {
  ...
  WifiOff, Map, List, Zap, Clock, Globe, AlertTriangle,
  ArrowUp, ArrowDown
} from 'lucide-react'
...
export default function NetworkPage() {
  ...
  const [deviceDetail, setDeviceDetail] = useState<NetDevice | null>(null)
  const [sortField, setSortField] = useState<'name' | 'status' | 'latency' | ''>('')
  ...
  const approveDevice = async (id: string) => {
    try {
      await api.post(`/devices/approve/${id}`)
      addToast('Device approved', 'success')
      load()
    } catch { addToast('Failed to approve device', 'error') }
  }

  const denyDevice = async (id: string) => {
    try {
      await api.post(`/devices/deny/${id}`)
      addToast('Device denied', 'success')
      load()
    } catch { addToast('Failed to deny device', 'error') }
  }
  ...
  const offlineCount = devices.filter(d => d.status !== 'online' && d.status !== 'approved').length
  ...
  const toggleSort = (field: 'name' | 'status' | 'latency') => {
    if (sortField === field) {
        ...
        <>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              <div className="glass-card" style={{ padding: '8px 16px', display: 'flex', alignItems: 'center', gap: 6, background: 'var(--glass-bg)' }}>
                <Server size={14} style={{ color: 'var(--accent)' }} />
                <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Total</span>
                <span style={{ fontSize: 14, fontWeight: 600 }}>{totalDevices}</span>
              </div>
              <div className="glass-card" style={{ padding: '8px 16px', display: 'flex', alignItems: 'center', gap: 6, background: 'var(--success-dim)' }}>
                <Check size={14} style={{ color: 'var(--success)' }} />
                <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Online</span>
                <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--success)' }}>{onlineCount}</span>
              </div>
              <div className="glass-card" style={{ padding: '8px 16px', display: 'flex', alignItems: 'center', gap: 6, background: 'var(--glass-bg)' }}>
                <WifiOff size={14} style={{ color: 'var(--text-muted)' }} />
                <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Offline</span>
                <span style={{ fontSize: 14, fontWeight: 600 }}>{offlineCount}</span>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                <button className={`btn btn-sm ${sortField === 'name' ? 'btn-primary' : 'btn-ghost'}`} onClick={() => toggleSort('name')} style={{ fontSize: 11 }}>
                  Name {sortField === 'name' && (sortDir === 'asc' ? <ArrowUp size={10} /> : <ArrowDown size={10} />)}
                </button>
                <button className={`btn btn-sm ${sortField === 'status' ? 'btn-primary' : 'btn-ghost'}`} onClick={() => toggleSort('status')} style={{ fontSize: 11 }}>
                  Status {sortField === 'status' && (sortDir === 'asc' ? <ArrowUp size={10} /> : <ArrowDown size={10} />)}
                </button>
                <button className={`btn btn-sm ${sortField === 'latency' ? 'btn-primary' : 'btn-ghost'}`} onClick={() => toggleSort('latency')} style={{ fontSize: 11 }}>
                  Latency {sortField === 'latency' && (sortDir === 'asc' ? <ArrowUp size={10} /> : <ArrowDown size={10} />)}
                </button>
              </div>
            </div>
          </div>
                ...
                return (
                  <div key={d.id} className="glass-card" style={{
                    ...
                    <div style={{
                      ...
                      background: online ? 'var(--success-dim)' : pending ? 'var(--warning-dim)' : 'var(--glass-bg)',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      color: online ? 'var(--success)' : pending ? 'var(--warning)' : 'var(--text-muted)', flexShrink: 0
                    }}>
                      <Icon size={16} />
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      {pending ? (
                        <span style={{ fontSize: 10, color: 'var(--warning)', padding: '2px 6px', borderRadius: 4, background: 'var(--warning-dim)' }}>Pending</span>
                      ) : (
                        <>
                          <span style={{
                    ...
                    {pending && (
                      <div style={{ display: 'flex', gap: 4 }} onClick={e => e.stopPropagation()}>
                        <button className="btn btn-success btn-sm" style={{ height: 28, fontSize: 11 }} onClick={() => { approveDevice(d.id); setConfirmAction(prev => ({ ...prev, open: false })) }}><Check size={12} /> Approve</button>
                        <button className="btn btn-danger btn-sm" style={{ height: 28, fontSize: 11 }} onClick={() => { denyDevice(d.id); setConfirmAction(prev => ({ ...prev, open: false })) }}><X size={12} /> Deny</button>
                      </div>
                    )}
                  </div>
                )
          ...
          {deviceDetail && (
            <div style={{ position: 'fixed', inset: 0, zIndex: 10000, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setDeviceDetail(null)}>
              <div className="glass-card" style={{ padding: 24, maxWidth: 400, width: '90%' }} onClick={e => e.stopPropagation()}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
                  {(() => { const Icon = getIcon(deviceDetail.type); return <Icon size={20} style={{ color: 'var(--accent)' }} /> })()}
                  <h4 style={{ fontSize: 15, fontWeight: 600 }}>{deviceDetail.name}</h4>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: 13 }}>
                  <div><span style={{ color: 'var(--text-muted)' }}>IP:</span> {deviceDetail.ip || 'N/A'}</div>
                  <div><span style={{ color: 'var(--text-muted)' }}>MAC:</span> {deviceDetail.mac || 'N/A'}</div>
                  <div><span style={{ color: 'var(--text-muted)' }}>Type:</span> {deviceDetail.type}</div>
                  <div><span style={{ color: 'var(--text-muted)' }}>Status:</span> {deviceDetail.status}</div>
                  <div><span style={{ color: 'var(--text-muted)' }}>Last Seen:</span> {deviceDetail.last_seen || 'N/A'}</div>
                  {deviceDetail.latency !== undefined && <div><span style={{ color: 'var(--text-muted)' }}>Latency:</span> {deviceDetail.latency}ms</div>}
                </div>
                <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
                  <button className="btn btn-ghost btn-sm" onClick={() => setDeviceDetail(null)}>Close</button>
                </div>
              </div>
            </div>
          )}
