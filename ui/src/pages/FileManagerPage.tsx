      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 8 }}>
          {filteredFiles.map(f => {
            ...
            return (
              <div key={f.name} className="card-liquid" style={{
                padding: 16, cursor: f.type === 'directory' ? 'pointer' : 'default',
                textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6
              }} onClick={() => f.type === 'directory' && navigate(`${currentPath}/${f.name}`)}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          {filteredFiles.map(f => {
            ...
            return (
              <div key={f.name} className="glass-card" style={{
                padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 10,
                cursor: isDir ? 'pointer' : 'default'
              }} onClick={() => isDir && navigate(`${currentPath}/${f.name}`)}>
